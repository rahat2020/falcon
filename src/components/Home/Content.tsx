...
const Content = () => {
    ...
    const filteredStatus = filteredData?.filter((item: { launch_success: boolean }) =>
            ...
            <Container>
                ...
                <div className="d-flex justify-content-end align-items-center">
                    <Form.Group className="mb-3 text-secondary" id="formGridCheckbox">
                        <Form.Check type="checkbox" label="Show upcoming only" />
                    </Form.Group>
                </div>
                                        ...
                                        filteredStatus?.map((item: {
                                            ...
                                            <Col md={4} key={index}>
                                                <Card className="border-1 shadow-sm">
                                                    <Card.Img variant="top" src={item?.links?.mission_patch} loading="lazy" style={{ width: '100%', height: '250px', objectFit: 'contain' }} />
                                                    <Card.Body className="d-flex justify-content-center align-items-center flex-column">
                                                        <small className="text-secondary">Launch Date: {item.launch_date_utc.slice(0, 10)}</small>
